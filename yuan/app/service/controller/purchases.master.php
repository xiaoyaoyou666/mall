...
class action extends app
{
	...
	private function add()
	{
		if($this->ev->get('submit'))
		{
			...
			$message = array(
			    ...
			    "forwardUrl" => "index.php?purchase-master-contents&catid={$args['contentcatid']}"
			);
	...
	private function edit()
	{
		...
		if($this->ev->get('submit'))
		{
			...
			$message = array(
			    ...
			    "rel" => "",
			    "callbackType" => "forward",
			    "forwardUrl" => "index.php?purchase-master-contents&catid={$content['contentcatid']}&page={$page}{$u}"
			);
			exit(json_encode($message));
		}
		else
	...
	private function del()
	{
		...
		$message = array(
		    ...
		    "rel" => "",
		    "callbackType" => "forward",
		    "forwardUrl" => "index.php?purchase-master-contents&catid={$content['contentcatid']}&page={$page}"
		);
		exit(json_encode($message));
	}
	...
	private function lite()
	{
		...
		if($this->ev->get('modifycontentsequence'))
		{
			...
			$message = array(
				...
				"message" => "操作成功",
				"callbackType" => "forward",
			    "forwardUrl" => "index.php?purchase-master-contents&catid={$catid}&page={$page}{$u}"
			);
			exit(json_encode($message));
		}
		...
		elseif($this->ev->get('movecategory'))
		{
			...
			if($targetcatid)
			{
				...
				$message = array(
					...
					"message" => "操作成功",
					"callbackType" => "forward",
				    "forwardUrl" => "?purchase-master-contents&catid={$catid}&page={$page}{$u}"
				);
			}
			else
			$message = array(
			...
			if($targetcatid)
			{
				foreach($contentids as $key => $id)
				{
					if($id)
					{
						...
						$args = array('contentcatid' => $targetcatid,'contenttitle' => $content['contenttitle'],'contentmoduleid' => $content['contentmoduleid'],'contentthumb' => $content['contentthumb'],'contentlink' => 'index.php?purchase-app-content&contentid='.$content['contentid']);
						$args['contentuserid'] = $this->_user['sessionuserid'];
						$args['contentusername'] = $this->_user['sessionusername'];
