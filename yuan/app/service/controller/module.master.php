...
class action extends app
{
	...
	private function fields()
	{
		...
		if($this->ev->get('modifyfieldsequence'))
		{
			...
			$message = array(
			    ...
			    "forwardUrl" => "?purchase-master-module-fields&moduleid={$moduleid}"
			);
		...
		if($this->ev->get('insertfield'))
		{
			...
			if($id)
			{
				$message = array(
					...
					"message" => "操作成功",
					"callbackType" => "forward",
				    "forwardUrl" => "?purchase-master-module-fields&moduleid={$moduleid}&page={$page}"
				);
			}
			else
			{
	...
	private function modifyfield()
	{
		if($this->ev->get('modifyfieldhtml'))
		{
			...
			$message = array(
			    ...
			    "rel" => "",
			    "callbackType" => "forward",
			    "forwardUrl" => "?purchase-master-module-fields&moduleid={$field['fieldmoduleid']}"
			);
			exit(json_encode($message));
		}
	...
	private function delfield()
	{
		...
		$message = array(
			...
			"message" => "操作成功",
		    "callbackType" => "forward",
		    "forwardUrl" => "index.php?purchase-master-module-fields&moduleid={$moduleid}"
		);
		exit(json_encode($message));
	}
	...
	private function modify()
	{
		...
		if($this->ev->get('modifymodule'))
		{
			...
			$message = array(
				...
				"message" => "操作成功",
			    "callbackType" => "forward",
			    "forwardUrl" => "?purchase-master-module"
			);
			exit(json_encode($message));
		}
		else
		...
		if($this->ev->post('insertmodule'))
		{
			...
			if(!$errmsg)
			{
				$message = array(
					...
					"message" => "操作成功",
					"callbackType" => "forward",
				    "forwardUrl" => "index.php?purchase-master-module&page={$page}"
				);
				exit(json_encode($message));
			}
			else
