	...
	<div style="width:95%;padding:0.5rem;margin:auto;margin-bottom:4.8rem;margin-top:0.6rem;background-color:#FFFFFF;overflow:hidden;">
		<form action="index.php?item-app-order-createorder" method="post" data-target="payfor" data-page="payfor">
			...
			<table class="table table-hover table-bordered">
				...
				<tr>
					<td colspan="2">
						<span class="pull-right">运费：￥{x2;$order['orderfee']}元 商品合计：￥{x2;eval: echo $order['orderprice'] - $order['orderfee']}元</span>
					</td>
				</tr>
				<tr style="background-color:#8B8B8B;">
					<td colspan="2">
						<span class="text-danger pull-right" style="color:#FFFFFF;">合计：￥{x2;$order['orderprice']}元</span>
					</td>
				</tr>
			</table>
			<p>
				<button style="line-height:3rem;background-color:#F02CAF" onclick="javascript:callpay();" type="button" data-target="payfor" data-page="payfor" class="btn btn-danger btn-block">微信支付</button>
	...
	{
		var par = {'url':'index.php?user-center-orders','target':'orderlist','page':'orderlist'};
		submitAjax(par);
	}
    function jsApiCall()
    {
