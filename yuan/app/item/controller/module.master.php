...
class action extends app
{
	...
	private function brands()
	{
		...
		{
			$brands = $this->ev->get('brands');
			$args = array('modulebrands' => $brands);
		...
		else
		{
			...
			$modulebrands = unserialize($module['modulebrands']);
			$this->tpl->assign('brands',$brands);
			$this->tpl->assign('modulebrands',$modulebrands);
			$this->tpl->assign('module',$module);
			$this->tpl->display('module_brands');
		}
	}
	...
	private function fields()
	{
		...
		if($this->ev->get('modifyfieldsequence'))
		{
			...
			$message = array(
				...
				"message" => "操作成功",
				"callbackType" => "forward",
			    "forwardUrl" => "index.php?item-master-module-fields&moduleid={$moduleid}"
			);
			exit(json_encode($message));
		}
		else
		...
		if($this->ev->get('insertfield'))
		{
			...
			if($id)
			{
				$message = array(
					...
					"message" => "操作成功",
					"callbackType" => "forward",
				    "forwardUrl" => "index.php?item-master-module-fields&moduleid={$moduleid}&page={$page}"
				);
			}
			else
			{
	...
	private function modifyfield()
	{
		...
		if($this->ev->get('modifyfieldhtml'))
		{
			...
			$message = array(
			    ...
			    "rel" => "",
			    "callbackType" => "forward",
			    "forwardUrl" => "index.php?item-master-module-fields&moduleid={$field['fieldmoduleid']}"
			);
			exit(json_encode($message));
		}
	...
	private function modify()
	{
		...
		if($this->ev->get('modifymodule'))
		{
			...
			$message = array(
				...
				"message" => "操作成功",
			    "callbackType" => "forward",
			    "forwardUrl" => "index.php?item-master-module"
			);
			exit(json_encode($message));
		}
		else
	...
	private function updateStatus()
	{
		...
		$args = array();
		$args['fieldlock'] = $status;
        $result = $this->module->modifyFieldHtmlType($args,$fieldid);
		...
		if($this->ev->post('insertmodule'))
		{
			...
			if(!$errmsg)
			{
				$message = array(
					...
					"message" => "操作成功",
					"callbackType" => "forward",
				    "forwardUrl" => "index.php?item-master-module&page={$page}"
				);
				exit(json_encode($message));
			}
			else
	...
	{
		$moduleid = $this->ev->get('moduleid');
		$fileds = $this->module->getMoudleFields($moduleid);
