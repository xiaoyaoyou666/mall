...
class action extends app
{
    ...
    private function index()
    {
        ...
        $invoicearr = $this->invoice->getInvoiceById($orders['invoiceid']);
        ...
        $sapOrderarr = $this->sap->getSapByOrdersn($ordersn,$orders['orderuserid']);
        ...
        $args22[] = array("AND","ordersn = :ordersn",'ordersn',$ordersn);
        $orderSapInfoAll = $this->sapinfo->getSapOrderByArgs($args22);

        $userArr = $this->user->getUserById($orders['ordersalerid']);
        $company_name = $userArr["company_name"];
        //------查询采购信息sap结束-----------
        ...
        if($this->ev->get('createorder'))
        {
            ...
            $order['ordersn'] = date('YmdHis').$orders['ordersalerid'].rand(10000,99999);
            $order['orderstatus'] = 1;//-------下单就为待发货状态为2----张建超--
            //$order['status_qufen'] = 1;//-------下单就为待发货状态为2----张建超--
            $order['orderuserid'] = $this->_user['sessionuserid'];
            $order['ordercreatetime'] = TIME;
            $order['ordersalerid'] = $orders['ordersalerid'];
            $order['orderprice'] = $orders['orderprice'];
            $order['orderfee'] = $orders['orderfee'];
            $order['orderitems'] = $orders['orderitems'];
            $order['orderaddress'] = $addressinfo;
            $order['buy_message'] = strip_tags($args['message']);   //买家留言
            $order['ordertitle'] = $orders['ordertitle'];
        ...
        else
        {
            ...
            $this->tpl->assign('sapOrderInfoArr',$sapOrderInfoArr);
            $this->tpl->assign('invoicearr',$invoicearr);
