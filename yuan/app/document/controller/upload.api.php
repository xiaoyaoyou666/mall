...
class action extends app
{
	...
	private function index()
	{
		...
		if($fileurl)
		{
			...
			$fastUrlArr = $this->fastdfs->applydfs($fileurl);

            //开发人员，在本地设置RUNTIME为local
            if (RUNTIME == 'local')
            {
			...
			if (!empty($fastUrlArr))
			{
				$fileArr = [
					...
					'url'  => $fastUrlArr['fileUrl'],
					'localPath' => $fileurl,
					'fileName' => $upfile['name'],
				];
				$this->fastdfs->addFileInfo($fileArr);

				$str = '<script type="text/javascript">window.parent.CKEDITOR.tools.callFunction('.$fn.', \''.$fastUrlArr['fileUrl'].'\', \''.$message.'\');</script>';
