...
class action extends app
{
	...
	private function edit()
	{
		...
		else
		{
			...
			$this->tpl->assign('forms',$forms);
			$this->tpl->assign('parentcat',$parentcat);
			$this->tpl->display('content2_edit');
		}
	}
	...
	private function del()
	{
		...
		$result = $this->content->delContent($contentid);
		if($result){
			$message = array(
				'statusCode' => 200,
				"message" => "操作成功",
	...
	private function index()
	{
		...
		//首页banner
		$args1 = array();
		$args1[] = array("AND","contentcatid = :contentcatid",'contentcatid',1);
		$contents1 = $this->content->getContentList($args1,$page,10);



		//网格超市banner
		$args2 = array();
		$args2[] = array("AND","contentcatid = :contentcatid",'contentcatid',2);
		$contents2 = $this->content->getContentList($args2,$page,10);
		...
		$this->tpl->assign('contents1',$contents1);
		$this->tpl->assign('contents2',$contents2);
		$this->tpl->assign('parentcat',$parentcat);
		$this->tpl->assign('categories',$categories);
		$this->tpl->assign('page',$page);
		$this->tpl->display('content2');
	}


	private function updateStatus()
	{
		$contentid = $this->ev->get('id_value');
		$status = $this->ev->get('value');
		$args = array();
		$args['contentstatus'] = $status;
        $result = $this->content->modifyContent($contentid,$args);
	...
	{
		$contentid = $this->ev->get('id_value');
		$contentsequence = $this->ev->get('value');
		$args = array();
		$args['contentsequence'] = $contentsequence;
        $result = $this->content->modifyContent($contentid,$args);
