...
class pdosql
{
	...
	public function makeSelectInJoin($args,$tablepre = NULL, $inJoin = 'left')
	{
		if($tablepre === NULL)$tb_pre = $this->tablepre;
		else $tb_pre = $tablepre;
		//if($args[0] === false)$args[0] = '*';
		...
		if(is_array($tables))
		{
			if (!empty($inJoin))
			{
				$db_tables = "{$tb_pre}{$tables[0]} AS {$tables[0]} {$inJoin} join {$tb_pre}{$tables[1]} AS {$tables[1]} on {$tables[0]}.user_id = $tables[1].userid";
			}
			else
			{
				$db_tables = array();
