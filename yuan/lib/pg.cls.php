...
class pg
{
	...
	public function outPhonePage($pages,$cpage,$url = false,$separate = "&page=")
	{
		...
		$pageString = $this->pre.'<a class="current ym_on">'.'第一页'.'</a>'.$this->end;
		$pageString .= $this->pre.'<a class="total-page">共'.$this->number.'条数据</a>'.$this->end;
		if($cpage < $pages)
		{
	...
	public function outPage($pages,$cpage,$url = false,$separate = "&page=")
	{
		...
		elseif($pages <= 10)
		{
			for($p = 1; $p <= $pages; $p++)
			{
				if($p == $cpage)$t = $this->pre.'<a href="#" class="current ym_on">'.$p.'</a>'.$this->end;
		...
		}else
		{
			...
			{
				if($start == $cpage)$t = $this->pre.'<a href="#" class="current ym_on">'.$start.'</a>'.$this->end;
	 ...
	 * @return bool|string
	 */
	public function outPageWithQuery($pages,$cpage,$url = false,$separate = "&page=", $query = '')
	{
		if($this->isPhone)return $this->outPhonePage($pages,$cpage,$url,$separate);
		if(!$url)
		{
		...
		}else
		{
			...
			{
				if($start == $cpage)$t = $this->pre.'<a href="#" class="current ym_on">'.$start.'</a>'.$this->end;
				else $t = $this->pre.'<a target="'.$this->target.'" href="'.$url.$separate.$start.$qs.'">'.$start.'</a>'.$this->end;
				$pageString .= $t;
			}
			if(isset($endString))$pageString .= $endString;
			$pageString .= $this->pre.'<a target="'.$this->target.'" href="'.$url.$separate.$pages.$qs.'">&rsaquo;|</a>'.$this->end;
			$pageString .= $this->pre.'<a class="total-page">共'.$this->number.'条数据</a>'.$this->end;
			return $pageString;
		}
	}
