...
class module
{
	...
	public function getMoudleFields($moduleid,$userinfo = 1,$app = NULL,$off=false)
	{
		if(!$app)$app = $this->G->app;
		if($userinfo == 1){
			if ($off) {
				$data = array(false,'module_fields',array(array('AND',"fieldmoduleid = :moduleid",'moduleid',$moduleid),array('AND',"fieldlock = :fieldlock",'fieldlock',0), array('OR'," (fieldpublic = 1 AND fieldappid = :app)",'app',$app)),false,'fieldsequence DESC,fieldid DESC');
			} else {
			...
			if ($off) {
				$data = array(false,'module_fields',array(array('AND',"fieldmoduleid = :moduleid",'moduleid',$moduleid),array('AND',"fieldlock = :fieldlock",'fieldlock',0),array('OR'," (fieldpublic = 1 AND fieldappid = :app)",'app',$app),array('AND',"fieldlock = '0'")),false,'fieldsequence DESC,fieldid DESC');
			} else {
	...
	//整理模型所需的参数，除去多余参数
	public function tidyNeedFieldsPars($args,$moduleid,$userinfo,$app = NULL)
	{
		if(!$app)$app = $this->G->app;
		$tmp = array();
		foreach($this->G->make('config',$app)->fields as $p)
		{
	...
	public function insertModuleField($args)
	{
		...
		$itemAddField = $this->insertModuleFieldData($args);
        if($itemAddField)
        {
	...
	//获取条件中的模型列表
	public function getModuleList($args,$page,$number = 20)
	{
		$data = array(
			'select' => false,
			'table' => 'module',
