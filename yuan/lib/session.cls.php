...
class session
{
	 ...
	 * @return mixed
	 */
	public function updateSessionRoleByUid($uid,$userRole)
	{
		$args['sessionuserrole'] = $userRole;
		$data = array(
			'table' => 'session',
			'value' => $args,
			'query' => array(array('AND', 'sessionuserid = :sessionuserid', 'sessionuserid', $uid))
		);
	 ...
	 * @return mixed
	 */
	public function getSessionByUserId($uid)
	{
		$data = array(false,'session',array(array('AND',"sessionuserid = :sessionuserid",'sessionuserid',$uid)));
		$sql = $this->pdosql->makeSelect($data);
		return $this->db->fetch($sql);
	}
